...
- Timeseries data. These might be recordings from sensors in an experiment, or an oscilloscope in the lab. Like in :ref:`Lab E <lab_e>`, we'll generate some synthetic data and put this into a dataframe, but we could equally load in data from an external :console:`.mat` or :console:`.csv` file.

Also, we'll run this file interactively. That is, we wont' use a :python:`if __name__ == "__main__":` block, but instead put everything in the global scope. We covered this in :ref:`Lab B <lab_b_stage_2>`. This style is quite common with data science type programmers. We're going to use it here as it means we can use the Jupyter notebook variable explorer to see the contents of the dataframe, which is useful when just starting out as the debugger don't have a good way of visualizing dataframes yet.
...
^^^^^^^^^^^^^^^^^^^^^^^
In your Lab F folder there is a file called :console:`marks.xlsx` in the :console:`data` folder. This contains the same marks as we used in :ref:`part 1 of the lab <lab_f_stage_1>`, but stored in a more helpful format. (Usually we want to separate out data from code, rather than hardcoding data into the code itself).
...
Selecting data
^^^^^^^^^^^^^^
Work through the simple examples below to get a feel for how to access data in a Polars dataframe.

#. To access a column of data you use the :python:`.select()` function. For example, to get the :python:`Course 1 mark` column:
...
#. To access a row of data you use the :python:`.filter()` function. For example, to get the data for the student with ID :python:`12345`:
...
#. Again, you can combine this with other commands, for example to get the IDs of all students who failed both Course 1 and Course 2:
...
#. was the mean in Course 1 amongst students who got a first?
...
#. :python:`.with_columns()` is used when we want to make changes to the dataframe. 
   ...
   we can use the :python:`.fill_null()` function. For example, to fill in missing data in the :python:`Course 2 late days` column with :python:`0`:
...
#. Changing one value in the dataframe has a slightly complicated syntax. To set the mark of student 12345 for Course 1 to 100:
   ...
   .. code-block:: python
      ...
      df = df.with_columns(pl.when(pl.col("Student ID") == 12345).then(100).otherwise(pl.col("Course 1 mark")).alias("Course 1 mark"))
   ...
   - Uses :python:`.otherwise()` to leave all other values unchanged by setting them to what is already in :python:`pl.col("Course 1 mark")`.
   - Uses :python:`.alias()` to specify which column is being changed.

#. This same syntax can be used to make new columns. For example, to apply a 10% late penalty to students who submitted late for Course 2:
...
- Each project has a unique ID number. For example: Project 1: Design heart rate estimation algorithm in Python.
- There are a couple of hundred different projects. 
...
Load the data into a Polars dataframe, and then answer the following questions. The first few you should be able to do using only the notes above. The latter ones will require some external research and might be quite hard. 
...
#. Copy the code below into a new file and run it. 
   ...
   The dataframe is storing times as a `datetime data type <https://uom-eee-eeen11202.github.io/notes-part1/chapters/programming_fundamentals/datetimes.html>`_. This is quite a high time resolution time series, the sampling rate is 1000 Hz. Some time series might update much closer than this. For example, stock prices might update every second. 
   ...
   #. Polars provides a number of *rolling* functions for time series data, which apply to a window of data. For example, a moving average filter averages the points in a window to smooth out short term fluctuations. An 11 tap moving average filter can be applied with:
      ...
      This adds another column to the dataframe, now called filtered. The filtered signal will look like
   ...
   .. admonition:: Solution
      ...
      A very small window means that the signal isn't smoothed very much. We need a good number of samples in order to be able to tell what the trye underlying average actually is. Too many samples however, and we start averaging out the underlying sine wave. It has an average of zero, and so if we take a mean of lots of points, we end up with something close to zero.

   #. There are a number of rolling functions available. Using :python:`.rolling_max()` find the maximum value of :python:`noisy` in a 1 second moving window. 
