...
.. quizdown::
   ...
   ## Which command activates a virtual environment called :console:`venv`?
   
   1. [x] :python:`source .venv/bin/activate` 
   ...
   ## Why can we use :python:`import math` in our Python code directly, whereas to use :python:`import pandas` we first need to add a module to the virtual environment?

   1. [ ] This is incorrect. Both :ptyhon:`math` and :python:`pandas` can be used without any further actions.
       > That's not correct. The statement in the question is right Try again. 
   1. [ ] This is incorrect. Both :python:`math` and :python:`pandas` need to be added to a virtual environment before they can be used.
       > That's not correct. The statement in the question is right Try again. 
   1. [ ] :python:`import math` doesn't actually do anything, the same functions are present even without this line of code.
       > That's not correct. :python:`import math` adds the ability for Python to use a wide number of common mathematical functions, such as :python:`math.sin()`, which couldn't be used in the code without this statement.
   1. [ ] :python:`pandas` is only available on some computer platforms.
         > That's not correct. :python:`pandas` is available on all platforms that support Python, but it is not part of the `standard library <https://uom-eee-eeen11202.github.io/notes-part1/chapters/software_development_tools/libraries.html#standard-library>`_.
   1. [X] :python:`math` is part of the `standard library <https://uom-eee-eeen11202.github.io/notes-part1/chapters/software_development_tools/libraries.html#standard-library>`_ installed with Python, and :python:`pandas` is not.
       > That's correct! 
   ...
   1. [ ] If a printer is connected to the computer, it prints out the Python file.
       > That's not correct. :python:`print()` doesn't have any relationship with using a physical printer. 
   ...
   1. [ ] It is not valid Python to do the sum :python:`i*mult` within the :python:`print()` statement.
   ...
   1. [x] The print statement is not indented. In Python, indentation is used to define blocks of code, such as the body of a loop or function.
       > That's correct!
   1. [ ] There should not be a colon :python:`:` at the end of the :python:`for` statement.
       > That's not correct. This is required Python syntax. 
   1. [ ] :python:`range()` is not a built-in Python function and so an :python:`import` statement is needed.
       > That's not correct. :python:`range()` is a core Python function and can be used without an :python:`import` statement.
   1. [ ] :python:`i` represents a complex number, it can't be used to represent something else.
       > That's not correct. :python:`1i` represents a complex number, to differentiate it from :python:`i` which can be the name of a variable if you want. Indeed :python:`i` is commonly used to count how many times a loop has run. 
   ...
   1. [ ] List
       > That's not correct. Refresh yourself on `storing multiple values <https://uom-eee-eeen11202.github.io/notes-part1/chapters/programming_fundamentals/variables.html#storing-multiple-values>`_
   1. [x] Tuple
       > That's correct!
   ...
   - [x] :python:`1i`
   - [x] :python:`1j`
   - [x] :python:`complex(0,1)`
   - [x] :python:`cmath.exp(1i)`
   - [x] :python:`cmath.exp(1j)`
   ...
   - [x] It will display "x is less than 0"
      > That's correct! It's probably not what the writer wanted the code to do, because :python:`x` is 1 here! The print statement should probably be indented to make it part of the if statement. At the moment the if statement only contains the :python:`x = 2` command.
   - [ ] It won't display anything
      > That's not correct It's probably what the writer wanted the code to do, because :python:`x` is 1 here! The print statement should probably be indented to make it part of the if statement. At the moment the if statement only contains the :python:`x = 2` command.
   - [ ] It will set :python:`x` to 2.
      > That's not correct. :python:`x = 2` is within the if statment, which won't run in this case because :python:`x = 1`. 
   - [x] The value of :python:`x` will stay at 1
      > That's correct!
   - [ ] It will set :python:`x` to a number less than 0.
      > That's not correct. :python:`if x < 0` compares :python:`x` to 0, but doesn't change the value of :python:`x`
